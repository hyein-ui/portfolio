...
function initResponsive() {
  ...
  setResponsive();
  mqMiddle.addEventListener("change", setResponsive);
  mqLarge.addEventListener("change", setResponsive);
  ...
  function runMqSmall() {
    ...
    document.querySelector(".ham").setAttribute("aria-expanded", false);
  }

  function runMqMiddle() {
    document.getElementById("mainNav").hidden = true;
    document.querySelector(".ham").setAttribute("aria-expanded", false);
  }
...
function initCommon() {
  /* --- PC웹 보기 링크 제어 --- */
  const arrPcVerLinks = document.querySelectorAll(".link-pc-ver");
  ...
  /* --- 태블릿웹 보기 링크 제어 --- */
  const arrTabVerLinks = document.querySelectorAll(".link-tab-ver");
  ...
  /* --- 모바일웹 보기 링크 제어 --- */
  const arrMoVerLinks = document.querySelectorAll(".link-mo-ver");
...
function initGSAP() {
  ...
  //현재 구조상 분기별 하나씩이 아니라 실행 순서대로 add해야 정상작동
  gsapMM.add("all", () => {
    headerAnimation();
    heroAnimation();
  });
